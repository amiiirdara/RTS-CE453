...
def run_bratley_algorithm():
    ...
    while queue:
        ...
        if not remaining_tasks:
            ...
            for task in task_sequence:
                ...
                print(
                    f"         Start: {task['start']}, Completion: {task['completion']}"
                )
        ...
        for index in range(len(remaining_tasks)):
            ...
            if current_task is None:
                ...
                task_candidate["completion"] = (
                    task_candidate["arrival"] + task_candidate["execution"]
                )
            else:
                task_candidate["prev_task"] = current_task
                task_candidate["start"] = max(
                    task_candidate["arrival"], current_task["completion"]
                )
                task_candidate["completion"] = (
                    task_candidate["start"] + task_candidate["execution"]
                )
    ...
    for task in solution_sequence:
        ...
        print(
            f"       Start: {task['start']}, Completion: {task['completion']}, Lateness: {lateness}"
        )
